...
public class AppointmentModel {
  ...
  public void setNewAppointment() {
    Appointment appointment = new Appointment(new Timestamp(System.currentTimeMillis()),
    ...
    if (this.appointment.getStatus() != AppointmentStatus.FINISHED) {
      if (this.isEditMode()) {
        ...
        this.appointmentService
            .createAppointment(this.appointment.getPatient(), this.appointment.getStaff(),
                this.appointment.getDescription(), this.appointment.getTitle(),
                this.appointment.getDate(), this.appointment.getEndDate());
      }
    }
  }
  ...
  public void moveAppointment(Date start) {
    long timeDifference =
        Date.from(appointment.getEndDate().atZone(ZoneId.systemDefault()).toInstant()).getTime()
            - Date.from(appointment.getDate().atZone(ZoneId.systemDefault()).toInstant()).getTime();
  ...
  public boolean isEditMode() {
    return this.appointment.getId() != 0
        && this.appointment.getStatus() != AppointmentStatus.FINISHED;
  }
  ...
  /**
   * Get the list of appointments that are form the currently set user and between the set time
   * range
   *
   * @return the list of appointments
   */
  public List<Appointment> getAppointmentList() {
    return this.appointmentService.findAppointmentsByStaffAndDateRange(this.staff,
        this.start.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime(),
        this.end.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime());
  }
