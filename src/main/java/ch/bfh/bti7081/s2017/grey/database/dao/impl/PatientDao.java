...
public class PatientDao extends GenericDaoImpl<Patient> implements GenericDao<Patient> {
  ...
  public boolean hasDrug(Patient patient, Drug drug) {
    CriteriaBuilder criteriaBuilder = em.getCriteriaBuilder();
    CriteriaQuery<PatientDrugAssociation> criteriaQuery = criteriaBuilder
        .createQuery(PatientDrugAssociation.class);
    Root<PatientDrugAssociation> patientDrugAssociationRoot = criteriaQuery
        .from(PatientDrugAssociation.class);
    criteriaQuery.select(patientDrugAssociationRoot)
        .where(criteriaBuilder.equal(patientDrugAssociationRoot.get("patientId"), patient.getId()),
  ...
  public void removeDrugFromPatient(Patient patient, Drug drug) {
    CriteriaBuilder criteriaBuilder = em.getCriteriaBuilder();
    CriteriaDelete<PatientDrugAssociation> delete = criteriaBuilder
        .createCriteriaDelete(PatientDrugAssociation.class);
    Root<PatientDrugAssociation> e = delete.from(PatientDrugAssociation.class);
    delete.where(criteriaBuilder.equal(e.get("patientId"), patient.getId()),
        criteriaBuilder.equal(e.get("drugId"), drug.getId()));
  ...
  public void removeHabitFromPatient(Patient patient, Habit habit) {
    CriteriaBuilder criteriaBuilder = em.getCriteriaBuilder();
    CriteriaDelete<PatientHabitAssociation> delete = criteriaBuilder
        .createCriteriaDelete(PatientHabitAssociation.class);
    Root<PatientHabitAssociation> e = delete.from(PatientHabitAssociation.class);
    delete.where(criteriaBuilder.equal(e.get("patientId"), patient.getId()),
        criteriaBuilder.equal(e.get("habitId"), habit.getId()));
